...
`docker-compose.yml` 定义 5 个核心服务：

1. `postgres`：业务主库（课程、作业、提交、成绩、题库等）。
2. `redis`：任务/缓存基础组件（AI 队列、前缀缓存索引等）。
3. `server`：NestJS 主业务服务，提供 `/api/v1` REST 接口。
4. `assistant_service`：AI 助手服务，提供 `/assistant/*` 接口。
5. `web`：Caddy 反向代理 + 前端静态资源托管 + HTTPS。

流量路径：

1. 浏览器请求 `110-lab.cn`。
2. Caddy 转发 `/api/*` 到 `server:3000`。
3. Caddy 转发 `/assistant/*` 到 `assistant_service:4100`。
4. Caddy 转发 `/uploads/*` 到 `server:3000`。
5. 其余请求返回前端静态页面。
...
1. `server/docker/entrypoint.sh`：启动前等待数据库并自动运行迁移。
2. Caddy 自动 HTTPS（域名生效后自动申请证书）。
3. 上传目录通过 volume 持久化，避免容器重建丢失。
