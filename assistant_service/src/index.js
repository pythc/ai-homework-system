...
若统计提示（scoreHint.meta.assignments）里已有作业数据，必须据此回答，不能说“暂无作业”。
教师查询“开放作业/当前作业列表”时，即使未指定 courseId，也应先返回其全部可访问课程下的作业汇总。
...
const normalizeLegacyMeta = (meta) => {
  const raw = meta && typeof meta === 'object' ? meta : {};
  const rawCounts = raw?.counts && typeof raw.counts === 'object' ? raw.counts : {};
  const rawCourses = clampArray(raw?.courses, 20);
  const rawAssignments = clampArray(raw?.assignments, 40);
  const rawStudents = clampArray(raw?.students, 20);

  return {
    counts: {
      courses: Number.isFinite(Number(rawCounts?.courses))
        ? Number(rawCounts.courses)
        : null,
      assignments: Number.isFinite(Number(rawCounts?.assignments))
        ? Number(rawCounts.assignments)
        : null,
      students: Number.isFinite(Number(rawCounts?.students))
        ? Number(rawCounts.students)
        : null,
    },
    ...
    assignments: rawAssignments.map((item) => ({
      ...
      totalScore: Number.isFinite(Number(item?.totalScore))
        ? Number(item.totalScore)
        : null,
      aiEnabled:
        typeof item?.aiEnabled === 'boolean' ? item.aiEnabled : null,
    })),
    studentCountsByCourse: rawStudents.map((item) => ({
      courseId: item?.courseId ?? null,
      studentCount: Array.isArray(item?.students) ? item.students.length : 0,
    })),
  };
};
...
const normalizeLegacyStats = (stats) => ({
  ...
  meta: normalizeLegacyMeta(stats?.meta),
});
