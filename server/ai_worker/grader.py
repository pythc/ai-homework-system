...
- 若从前提到结论跨越过大、缺少关键中间步骤或逻辑桥接：
  1) uncertaintyReasons 追加 {"code":"JUMP_STEP","message":"存在关键推导跳步，建议教师复核"}；
  2) 对相关 rubric 项按“过程完整性不足”适度扣分，并在 reason 写清缺失步骤。
"""
...
- 若存在矛盾：
  1) uncertaintyReasons 追加 {"code":"STEP_CONFLICT","message":"作答步骤存在前后矛盾，建议教师复核"}；
  2) 若最终答案与过程矛盾明显，可再追加 FINAL_ANSWER_MISMATCH；
  3) 按 rubric 对逻辑一致性与结论可信度扣分。
"""
...
- 若必要步骤缺失：
  1) uncertaintyReasons 追加 {"code":"MISSING_INFO","message":"缺少必要步骤或关键信息，建议教师复核"}；
  2) 在对应 rubricItem 的 reason 中明确“缺失了哪些必要步骤”；
  3) 扣分需与缺失程度匹配，避免一刀切清零。
"""
