...
- options 含 minConfidence、returnStudentMarkdown、handwritingRecognition、gradingStrictness、customGuidance，
  以及 plagiarismDetection、jumpStepDetection、stepConflictDetection、requiredStepDetection。
...
- 图片无法辨认/题号无法对应等无法稳定判分场景，必须给出相应 uncertaintyReasons。
- “跳步 / 步骤矛盾 / 缺少必要步骤”优先写入 item.reason 作为扣分依据，不作为 uncertaintyReasons。

【可用 code】
UNREADABLE, JUMP_STEP, STEP_CONFLICT, FINAL_ANSWER_MISMATCH, MISSING_INFO, FORMAT_AMBIGUOUS, LOW_CONFIDENCE, NON_HANDWRITTEN, PLAGIARISM_RISK
...
PLAGIARISM_PROMPT_SUFFIX = """

========================
十一、抄袭识别（仅在 plagiarismDetection=true 时启用）
========================
- 目标是“风险识别”，不是“定案”。仅在证据充分时标记风险。
- 优先从以下信号综合判断：与标准答案大段同构复述、步骤措辞异常一致且缺少个体化推导、与题目无关但模板化内容复用。
- 若怀疑抄袭风险：
  1) uncertaintyReasons 追加 {"code":"PLAGIARISM_RISK","message":"检测到疑似抄袭或高度模板化作答，建议教师复核"}；
  2) result.isUncertain=true；
  3) 在 result.comment 中明确写“存在疑似抄袭风险，仅供教师复核”。
- 证据不足时禁止强行判定抄袭，不得因主观猜测直接大幅扣分。
"""

JUMP_STEP_PROMPT_SUFFIX = """

========================
十二、跳步识别（仅在 jumpStepDetection=true 时启用）
========================
- 对 CALCULATION / PROOF / SHORT_ANSWER 重点检查推导连续性。
- 若从前提到结论跨越过大、缺少关键中间步骤或逻辑桥接：
  1) 对相关 rubric 项按“过程完整性不足”适度扣分；
  2) 在对应 item.reason 明确写出“跳过了哪些关键步骤”；
  3) 不要仅因跳步把该题标记为 isUncertain。
"""

STEP_CONFLICT_PROMPT_SUFFIX = """

========================
十三、步骤矛盾识别（仅在 stepConflictDetection=true 时启用）
========================
- 检查同一作答内部是否自相矛盾：前后公式冲突、符号约定冲突、过程与最终结论不一致。
- 若存在矛盾：
  1) 按 rubric 对逻辑一致性与结论可信度扣分；
  2) 在对应 item.reason 明确“矛盾点在哪一步”；
  3) 若最终答案与过程矛盾明显，可追加 FINAL_ANSWER_MISMATCH；
  4) 不要仅因步骤矛盾把该题标记为 isUncertain。
"""

REQUIRED_STEP_PROMPT_SUFFIX = """

========================
十四、必要步骤识别（仅在 requiredStepDetection=true 时启用）
========================
- 按题型与 rubric 识别“必要步骤”：关键公式引用、边界条件说明、核心论证节点、结论回代校验等。
- 若必要步骤缺失：
  1) 在对应 rubricItem 的 reason 中明确“缺失了哪些必要步骤”；
  2) 扣分需与缺失程度匹配，避免一刀切清零；
  3) 不要仅因必要步骤缺失把该题标记为 isUncertain。
"""
...
def build_system_prompt(
    ...
    jump_step_detection: bool = True,
    step_conflict_detection: bool = True,
    required_step_detection: bool = True,
) -> str:
    ...
    if plagiarism_detection:
        prompt += PLAGIARISM_PROMPT_SUFFIX
    if jump_step_detection:
        prompt += JUMP_STEP_PROMPT_SUFFIX
    if step_conflict_detection:
        prompt += STEP_CONFLICT_PROMPT_SUFFIX
    if required_step_detection:
        prompt += REQUIRED_STEP_PROMPT_SUFFIX
...
def extract_options_payload(json_payload: Dict) -> Dict:
    ...
    return {
        ...
        "plagiarismDetection": options.get("plagiarismDetection", True),
        "jumpStepDetection": options.get("jumpStepDetection", True),
        "stepConflictDetection": options.get("stepConflictDetection", True),
        "requiredStepDetection": options.get("requiredStepDetection", True),
...
def main() -> int:
    ...
    plagiarism_detection = bool(options_payload.get("plagiarismDetection", True))
    jump_step_detection = bool(options_payload.get("jumpStepDetection", True))
    step_conflict_detection = bool(options_payload.get("stepConflictDetection", True))
    required_step_detection = bool(options_payload.get("requiredStepDetection", True))
    ...
    system_prompt = build_system_prompt(
        ...
        jump_step_detection,
        step_conflict_detection,
        required_step_detection,
    )
